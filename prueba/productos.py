...
#Funcion Importar(desde cvs)
def importarCvs():
    with open('productos4.csv','r') as archivo:
...
def insertarProductos():
    lista=[]
    con = modelo.Conectar()
    itemsSeleccionados = tabla.selection()
    data = tabla.item(itemsSeleccionados)["values"]
    for dato in data:
        lista.append(dato)
        print(lista)
...
def visualizarWeb():
    webbrowser.open_new('https://app.powerbi.com/view?r=eyJrIjoiNTZmZTUyZTUtZjY4MC00MDBjLTkxMjgtY2I0Y2U0OWFlNGMyIiwidCI6ImVjNmU3NTQ4LWZjZTMtNGY0NC05NjZhLWY0N2EwZjEyNWE4MSIsImMiOjR9')

#creamos la ventana
ventana = Tk()

ventana.geometry('1000x417')
...
importarBtn = Button(text='Importar',bg='tan1',font=('calibi',12,'bold'),command=importarCvs)
importarBtn.place(x=286, y=250, width=84, height=30)

insertarBtn = Button(text='Insertar', bg='tan1', font=('calibi',12,'bold'), command=insertarProductos)
insertarBtn.place(x=400, y=250, width=84, height=30)
...
visualizarBtn = Button(text='Visualizar Datos', bg='tan1',font=('calibi',13,'bold'),command=visualizarWeb)
